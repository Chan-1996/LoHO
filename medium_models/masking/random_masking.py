...
class RandomMasking:
    def __init__(self, model, masking_prob, mask_type, fo_optim_layers, fp16):
        self.model = model
        ...
        for key, _ in self.model.named_parameters():
            if self.fo_optim_layers is not None and len(self.fo_optim_layers) > 0:
                for layer in self.fo_optim_layers:
                    if layer in key:
                            self.convert_masking_layer(key)
                            break
            else:
                self.convert_masking_layer(key)
    ...
    def convert_masking_layer(self, key):
        if 'weight' in key and 'embed' not in key and 'lm_head' not in key and 'norm' not in key and 'Norm' not in key:
            ...
            if self.mask_type == 'random':
                rand_mat = torch.rand(out_dim, in_dim)
                mask = rand_mat > self.masking_prob
                self.mask_dict[key] = mask
                logger.info(f"true masking prob: {torch.sum(mask) / torch.prod(torch.tensor(mask.shape))}")
            elif self.mask_type == 'absolute_max':
                weight = module.weight
                keep_num = int(torch.prod(torch.tensor(weight.shape)).item() * self.masking_prob)
                assert keep_num > 0
                top_pos = torch.topk(weight.view(-1).abs(), keep_num, largest=True)[1]
                mask = torch.zeros_like(weight.view(-1))
                mask[top_pos] = 1.0
                mask = mask.reshape(weight.shape)
                self.mask_dict[key] = mask
            elif self.mask_type == 'absolute_min':
                weight = module.weight
                keep_num = int(torch.prod(torch.tensor(weight.shape)).item() * self.masking_prob)
                assert keep_num > 0
                top_pos = torch.topk(weight.view(-1).abs(), keep_num, largest=False)[1]
                mask = torch.zeros_like(weight.view(-1))
                mask[top_pos] = 1.0
                mask = mask.reshape(weight.shape)
                self.mask_dict[key] = mask
