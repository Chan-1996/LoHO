...
def get_label(task, line):
    if task in ["MNLI", "MRPC", "QNLI", "QQP", "RTE", "SNLI", "SST-2", "STS-B", "WNLI", "CoLA", "GLUE-SST-2"]:
        # GLUE style
...
def split_header(task, lines):
    ...
    if task in ["CoLA"]:
        return [], lines
    elif task in ["MNLI", "MRPC", "QNLI", "QQP", "RTE", "SNLI", "SST-2", "STS-B", "WNLI", "GLUE-SST-2"]:
...
def main():
    ...
    parser.add_argument("--task", type=str, nargs="+",
        default=['SST-2', 'sst-5', 'mr', 'cr', 'mpqa', 'subj', 'trec', 'CoLA', 'MRPC', 'QQP', 'STS-B', 'MNLI', 'SNLI', 'QNLI', 'RTE', 'GLUE-SST-2'],
        help="Task names")
    ...
    parser.add_argument("--mode", type=str, default='k-shot', choices=['k-shot', 'k-shot-10x', 'k-shot-1k-test', 'full-data'], help="k-shot or k-shot-10x (10x dev set)")
    ...
    for seed in args.seed:
        ...
        for task, dataset in datasets.items():
            ...
            if args.mode == 'full-data':
                setting_dir = os.path.join(task_dir, f"full-data")
            else:
            ...
            if task in ["MNLI", "MRPC", "QNLI", "QQP", "RTE", "SNLI", "SST-2", "STS-B", "WNLI", "CoLA", "GLUE-SST-2"]:
                ...
                if args.mode == 'full-data':
                    test_dataset.to_csv(os.path.join(setting_dir, 'dev.csv'), header=False, index=False)
