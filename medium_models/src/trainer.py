    ...
    def efficient_perturb_parameters(self, model: nn.Module, random_seed: int, scaling_factor=1):
        ...
        for name, param in self.named_parameters_to_optim:
            ...
            if mask is not None:
                ...
                param.data.view(-1)[index] += scaling_factor * z * self.args.zero_order_eps
    ...
    def train(self, model_path=None, dev_objective=None):
        ...
        for epoch in range(epochs_trained, int(num_train_epochs)):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if self.args.zero_order_optim:
                ...
                elif self.args.zo_combine_fo:
                    # combine zero-order optimization and sgd/adam optimization
                    # Get parameters that should be optimized.
                    if self.args.layer_wise_hybrid:
                        self.named_parameters_to_optim = []
                        self.fo_named_parameters_to_optim = []
                        for name, param in model.named_parameters():
                            is_fo_params = False
                            for name1 in self.args.fo_optim_layers:
                                if name1 in name:
                                    self.fo_named_parameters_to_optim.append((name, param))
                                    is_fo_params = True
                                    break
                            if not is_fo_params:
                                self.named_parameters_to_optim.append((name, param))
                    ...
                    #calculate gradident using sgd/adam
                    model.train()
                    inputs = self._prepare_inputs(inputs)
                    with self.compute_loss_context_manager():
                        sgd_loss = self.compute_loss(model, inputs)

                    if self.args.n_gpu > 1:
                        sgd_loss = sgd_loss.mean()  # mean() to average on multi-gpu parallel training
                    tr_loss += sgd_loss
                    sgd_loss.backward()
                    # if self.args.optimizer == 'adam':
                    ...
                    # else:
                    #     if len(self.args.adam_optim_layers) > 1:
                    ...
                    for name, param in self.named_parameters_to_optim:
                        ...
                        if mask is not None:
                            ...
                            param.data.view(-1)[index] -= self.args.zo_learning_rate * (
                                    projected_grad * z + self.args.weight_decay * param.data.view(-1)[index])
                        else:
                            param.data = param.data - self.args.zo_learning_rate * (
                    ...
                    if (step + 1) % self.args.gradient_accumulation_steps == 0 or (
                        ...
                        if (self.args.logging_steps > 0 and self.state.global_step % self.args.logging_steps == 0) or (
                            ...
                            logs = {}
                            logs["zo_loss1"] = loss1.item()
                            logs["zo_loss2"] = loss2.item()
                            # tr_loss_scalar = tr_loss.item()
                            logs["fo_loss"] = sgd_loss.item()
                            # logs["sgd_or_adam_loss"] = (tr_loss_scalar - logging_loss_scalar) / self.args.logging_steps
                            # logging_loss_scalar = tr_loss_scalar
                            # if self.state.global_step % 100 == 0:
                            #     logs["sgd_or_adam_loss"] = tr_loss_scalar - logging_loss_scalar
                            #     logging_loss_scalar = tr_loss_scalar
                            # if not self.args.zero_order_clip_grad:
                            #     norm = 0.0
                            #     for _, p in model.named_parameters():
                            ...
                            #     norm = torch.sqrt(norm)
                            # logs["grad_norm"] = norm.item()
                            logs["zo_learning_rate"] = self.args.zo_learning_rate
                            logs["fo_learning_rate"] = (
                                scheduler.get_last_lr()[0]
                                if version.parse(torch.__version__) >= version.parse("1.4")
                                else scheduler.get_lr()[0]
                            )
                ...
                # standard, non-ZO optimization
                    ...
                    if self.args.fo_optim_layers is not None and not self.args.weight_wise_hybrid:
                        for name, param in model.named_parameters():
                            is_fo_params = False
                            for name1 in self.args.fo_optim_layers:
                                if name1 in name:
