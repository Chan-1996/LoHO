...
class DynamicTrainingArguments(TrainingArguments):
    ...
    zo_combine_fo: bool = field(
        default=False,
        metadata={'help': 'when on, trains the model by combining zero-order optimization and adam optimization'}
    )
    layer_wise_hybrid: bool = field(
        default=False,
        metadata={'help': 'whether to use layer wise zo and fo hybrid optimization'}
    )
    weight_wise_hybrid: bool = field(
        default=False,
        metadata={'help': 'whether to use weight wise zo and fo hybrid optimization'}
    )
    masking_prob: float = field(
        default=0.99,
        metadata={'help': 'ratio of parameters updated by zo optimizer'}
    )
    mask_type: str = field(
        default='random',
        metadata={'help': 'mask type: random, absolute_min or absolute_max'}
    )
    zo_learning_rate: float = field(
        default=1e-6,
        metadata={'help': 'learning rate of the zero order optimizer.'}
    )
    fo_optim_layers: List[str] = field(
        default=None,
        metadata={'help': 'parameters from which layers using zero-order optimization'}
    )
    ...
    efficient_zero_order: bool = field(
        default=False,
        metadata={
            "help": "Efficient zero-order: resample noise vectors instead of saving them. enable different model loading using --hf_inference_model"}
    )
    ...
    scale_lr_with_samples: bool = field(
        default=False,
        metadata={
            "help": "Scales the LR proportionally to the number of z samples. --learning_rate will be the LR for one z sample."}
    )
    zero_order_sample: int = field(
        default=1,
        metadata={
            "help": "Sample times for zero-order estimate. If scheduler is 'linear', this number is the max sample number."}
    )
    ...
    zo_by_layer: bool = field(
        default=False,
        metadata={
            "help": "For ZO: estimate the gradients on each layer individually, scales number of forward passes per grad step by a factor of L"}
    )
    ...
    recompute_norms: bool = field(
        default=False,
        metadata={
            'help': 'Recompute the grad or parameter norm (whichever was specified as --zo_variant) at the start of each epoch.'}
    )
    ...
    norm_running_update: bool = field(
        default=False,
        metadata={
            "help": "When performing --zo_by_layer and using --zo_variant 'grad_norm', update the layer grad norms as they are recomputed at each step"}
    )
    change_grad_estimate: bool = field(
        default=False,
        metadata={
            "help": "Changes the expectation of the ZO gradient estimate according to zo_variant, instead of just modifying the variance"}
    )
...
def main():
    ...
    if training_args.zo_combine_fo and training_args.weight_wise_hybrid:
        rm = RandomMasking(model, training_args.masking_prob, training_args.mask_type, training_args.fo_optim_layers,
                           fp16=False)
        training_args.mask_dict = rm.mask_dict
    else:
        if training_args.weight_wise_hybrid:
            rm = RandomMasking(model, training_args.masking_prob, training_args.mask_type,
                               training_args.fo_optim_layers, fp16=False)
            training_args.mask_dict = rm.mask_dict
