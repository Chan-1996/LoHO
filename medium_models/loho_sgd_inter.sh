...
TAG=${TAG:-k${K}-${MODEL_NAME}-mezo-sgd-last-12layer-linear-fo-lr-no-dropout-${EXTRA_TAG}}
...
TYPE=prompt GRID_TAG=$GR_TAG TAG=$TAG STEPS=$STEP TASK=$TASK SEED=$SEED MODEL=$MODEL K=$K \
      ...
      --lr_scheduler_type "constant" --optimizer "sgd" --efficient_zero_order \
      $@